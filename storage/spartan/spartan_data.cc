...
int Spartan_data::open_table(char *path)
{
    ...
    // open file as read/write mode
    // create the file if not found
    // treate file as binary, and use default flags
    data_file = my_open(path, O_RDWR | O_CREAT | 0, MYF(0));
    if (data_file == -1)
        DBUG_RETURN(errno);
    read_header();
    DBUG_RETURN(0);
...
long long Spartan_data::write_row(uchar *buf, int length)
{
    ...
    // Note: my_write() returns the uchars writen or -1 on error
    pos = my_seek(data_file, 0L, MY_SEEK_END, MYF(0));

    // Note: my_malloc take a size of memory to be allocated
    ...
    // Return new file pointer or return -1 if error from my write
...
long long Spartan_data::update_row(uchar *old_rec, uchar *new_rec, int length, long long position)
{
    ...
    pos = position;
    // if position unknow, scan for the record by reading a row at a time until
    // found
    if (position == -1) //don't know where it is ... scanf for it
    {
        cmp_rec = (uchar *)my_malloc(PSI_NOT_INSTRUMENTED, length, MYF(MY_ZEROFILL | MY_WME));
        pos = 0;
        // Note: my_seek() returns pos if no errors or -1 if error.
        cur_pos = my_seek(data_file, header_size, MY_SEEK_SET, MYF(0));
        // Note: read_row() returns current file pointer if no error or -1 if
        // error
    ...
    {
        /*
         * write the deleted uchar, the length of the row, and the data at the
...
int Spartan_data::delete_row(uchar *old_rec, int length, long long position)
{
     ...
     * found.
     */
    if (position == -1) // domn't know where it is...scan for it
    {
        cmp_rec  = (uchar *)my_malloc(PSI_NOT_INSTRUMENTED, length, MYF(MY_ZEROFILL | MY_WME));
        pos = 0;
        /*
         * Note: my_seek() return pos if no errors or -1 if error
         */
        cur_pos = my_seek(data_file, header_size, MY_SEEK_SET, MYF(0));
        /*
         * Note: read_row() returns current file pointer if no error or -1 if
    ...
    // if position found or provied, write the row.
    if (pos != -1)
    {
        /*
         * write the deleted uchar set to 1 which marks row as deleted at the
         * current file pointer.
         * Note" my_write() returns the uchars written or -1 on error
         */
...
int Spartan_data::read_row(uchar *buf, int length, long long position)
{
    ...
    /*
     * if my_seek found the position, read the deleted uchars.
