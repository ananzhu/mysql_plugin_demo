...
// PSI_memory_key Spartan_key_memory;
...
{
  SDE_NDX_NODE *p = nullptr;
  SDE_NDX_NODE *n = nullptr;
  SDE_NDX_NODE *o = nullptr;
  int i = -1;
  int icmp;
  bool dupe = false;
  bool done = false;
  ...
  */
  while ((p != nullptr) && !done)
  {
    icmp = memcmp(ndx->key, p->key_ndx.key,(ndx->length > p->key_ndx.length) ? ndx->length : p->key_ndx.length);
...
uchar *Spartan_index::get_next_key()
{
  ...
  {
    key = (uchar *)my_malloc(PSI_NOT_INSTRUMENTED,max_key_len, MYF(MY_ZEROFILL | MY_WME));
